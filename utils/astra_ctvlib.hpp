...
public: 
    ...
    int Nrow, Ncol, Nslice, Nproj, Ny, Nz, SMP;
    ...
    void FBP(bool apply_positivity=true);
    void CGLS(int nIter=1);
    // void fista_NAG(float beta);
    ...
    float matrix_2norm();
    void set_model_projections(Mat &in);
    float data_distance();
    float rmse();
    ...
    void fista_nesterov_momentum(float beta);
    float l1_norm();
    float least_squares();
    float get_lipschitz();
