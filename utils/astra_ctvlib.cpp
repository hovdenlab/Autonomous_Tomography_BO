...
void astra_ctvlib::initializePoissonML() {
    ...
    // Initialize Forward and Back-Projection Operator
...
// Take in projections from a python generated projection matrix
void astra_ctvlib::set_model_projections(Mat &in)
{
    g = in;
    SMP = 1;
}
...
float astra_ctvlib::tv_fgp_3D(int ng, float lambda) {  
    if (momentum) { return cuda_tv_fgp_3D(yk.data, ng, lambda, Nslice, Ny, Nz); }
    else          { return cuda_tv_fgp_3D(recon.data, ng, lambda, Nslice, Ny, Nz); } }
...
void astra_ctvlib::initialize_fista() {
    ...
    // Initialize Forward Projection Operators
...
float astra_ctvlib::least_squares() {
    float cost = 0;
    for (int s=0; s<Nslice; s++){
...
// Estimate Lipschitz for Least-Squares 
...
// void astra_ctvlib::initialize_ogm() {
//     momentum = true;
//     yk = Matrix3D(Nslice, Ny, Nz);
//     recon_old = Matrix3D(Nslice,Ny,Nz);
// }
...
PYBIND11_MODULE(astra_ctvlib, m)
{
    ...
    astra_ctvlib.def("set_model_projections", &astra_ctvlib::set_model_projections, "Take in projections from a python generated projection matrix");
    ...
    astra_ctvlib.def("tv", &astra_ctvlib::tv_3D, "Measure TV of reconstruction");
    ...
    astra_ctvlib.def("least_squares", &astra_ctvlib::least_squares,"Least Squares Reconstruction");
    astra_ctvlib.def("get_lipschitz", &astra_ctvlib::get_lipschitz,"Get Lipschitz Parameter");
    astra_ctvlib.def("remove_momentum", &astra_ctvlib::remove_momentum, "Remove Momentum");
}
...
//     // Gradient Descent
//     yk = SIRT(xk);
//     // forward_back_projection(yk); 

//     // Proximal Operator 
//     if( proximal == 'L1' || proximal == 'soft' ) {
//         yk.soft_threshold(lambda); }
//     else if( proximal == 'TV' || proximal == 'tv' ) { 
//         cuda_tv_fgp_3D(yk.data, ng, lambda, Nslice, Ny, Nz); 
//         yk.positivity();                                        }
//     memcpy(&xk.data[xk.index(s,0,0)], &yk.data[yk.index(s,0,0)], sizeof(float)*Nslice*Ny*Nz);

//     // Momentum 
//     tk = 0.5 * (1 + std::sqrt(1 + 4*tk0));
//     cuda_NAG(xk, xk_old, yk, (tk0 - 1) / tk); // Nesterov Momentum 
//     // for (int jj=0; jj < Nslice*Ny*Nz; jj++) {   
//     //     yk.data[jj] -= xk.data[jj] + (tk0 - 1) / tk * ( xk.data[jj] - xk_old.data[jj] ); }
//     memcpy(&xk_old.data[xk.index(s,0,0)], &xk.data[yk.index(s,0,0)], sizeof(float)*Nslice*Ny*Nz);
//     tk0 = tk;
// }
// memcpy(&recon.data[xk.index(s,0,0)], &xk.data[xk.index(s,0,0)], sizeof(float)*Nslice*Ny*Nz);
